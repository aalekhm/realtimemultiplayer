...
game_core.prototype.update = function(t) {
    ...
    this.dt = this.lastframetime ? ( (t - this.lastframetime)/1000.0).fixed() : 0.015;
    ...
    if(!target) {
     ...
     if(target && previous) {
        ...
        if(this.client_smoothing) {
            this.players.other.pos = this.v_lerp( this.players.other.pos, this.ghosts.pos_other.pos, 0.015*this.client_smooth);
        } else {
        ...
        if(!this.client_predict && !this.naive_approach) {
            ...
            if(this.client_smoothing) {
                this.players.self.pos = this.v_lerp( this.players.self.pos, local_target, 0.015*this.client_smooth);
            } else {
